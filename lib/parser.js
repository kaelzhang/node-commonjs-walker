...
parser.parse = function (path, options, callback) {
  fs.readFile(path, function (err, content) {
    ...
    parser._parse_js(content.toString(), function (err, ast) {
      if (err) {
        return callback({
          code: 'EPARSEJS',
...
parser._parse_js = function (content, callback) {
  var ast;
  try {
...
parser._parse_dependencies = function (node, host, options) {
  host || (host = []);
  ...
  if (
    ...
    if (arg1.type !== 'Literal') {
      parser._throw(strict, pos_text(arg1.loc.start) + 'Method `require` only accepts a string literal.' );
      return;
    }
  ...
  if (util.isArray(node)) {
    node.forEach(function (sub) {
      parser._parse_dependencies(sub, host, options);
    });

  } else {
    var key;
    for (key in node) {
      parser._parse_dependencies(node[key], host, options);
    }
  }
};
